...
FROM
    trackerSR_schedules s
    JOIN trackerSR_route r ON s.route_id = r.id
WHERE
    s.weekday_type = 'Weekday'
...
INSERT IGNORE INTO
    transactions (id, user_id, balance)
VALUES (1, 1, 0.00);

-- Query to get latest balance
SELECT balance
FROM transactions
WHERE
    user_id = ?
